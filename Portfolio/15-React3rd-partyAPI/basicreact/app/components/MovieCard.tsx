...
export default function MovieCard({ movie }: { movie: any }) {
  ...
  const handleLike = async (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
        const res = await fetch(`http://localhost:5001/api/movies/${movie._id}/like`, { method: 'POST' });
  ...
  const handleDislike = async (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
        const res = await fetch(`http://localhost:5001/api/movies/${movie._id}/dislike`, { method: 'POST' });
        const data = await res.json();
        setLikes(data.likes);
    } catch (err) {
        console.error(err);
